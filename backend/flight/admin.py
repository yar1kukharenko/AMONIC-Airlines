...
class AircraftAdmin(admin.ModelAdmin):
    list_display = ('id','name', 'make_model', 'total_seats', 'economy_seats', 'business_seats')
    search_fields = ('name', 'make_model')
    list_filter = ('make_model',)

@admin.register(Route)
class RouteAdmin(admin.ModelAdmin):
    list_display = ('id','departure_airport', 'arrival_airport', 'distance', 'flight_time')
    search_fields = ('departure_airport__name', 'arrival_airport__name')
    list_filter = ('departure_airport', 'arrival_airport')

@admin.register(Schedule)
class ScheduleAdmin(admin.ModelAdmin):
    list_display = ('id','route', 'aircraft', 'date', 'time', 'flight_number', 'economy_price', 'confirmed')
